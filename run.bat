@echo off
REM ARQV30 Enhanced v3.0 - Script de Execução Windows
REM Execute este arquivo para iniciar a aplicação com External AI Verifier
...
python -c "import flask, requests, google.generativeai, supabase, textblob, vaderSentiment" >nul 2>&1
if errorlevel 1 (
...
REM Verifica External AI Verifier
echo 🔍 Verificando External AI Verifier...
if exist "..\external_ai_verifier\src\external_review_agent.py" (
    echo ✅ External AI Verifier encontrado
) else (
    echo ⚠️ AVISO: External AI Verifier não encontrado
)

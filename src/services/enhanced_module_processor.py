...
"""
    ...
    def _generate_fallback_content(self, module_name: str, config: Dict[str, Any], base_data: Dict[str, Any]) -> str:
        ...
        fallback_contents = {
            'provas_visuais': self._generate_visual_proofs_fallback(base_data),
            'anti_objecao': self._generate_anti_objection_fallback(base_data),
            'metricas_conversao': self._generate_metrics_fallback(base_data),
            'estrategia_preco': self._generate_pricing_fallback(base_data),
            'canais_aquisicao': self._generate_acquisition_fallback(base_data)
        }
        
        if module_name in fallback_contents:
            return fallback_contents[module_name]
